...
function sqrt(a, res)
    ...
    elseif t == "f" then
        return prv.mpf_sqrt(a, res)
    elseif t == "z" then
		local v = prv.mpf_init()
		prv.mpf_set_z(v, a)
        return prv.mpf_sqrt(v, res)
    elseif t == "?" then
		error("unsupported type")
    end
    assert(a >= 0, "non-negative number expected")
    return prv.mpf_sqrt(dtof(a), res)
end
